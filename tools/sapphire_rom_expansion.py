...
# 3x pad, 20s source sha1, I header crc32, remainder reserved.
HEADER = struct.Struct("<8sHHIIIIIHHHH4sB3x20sI")
...
def build_control(native: dict) -> bytes:
    ...
    source_sha1 = bytes.fromhex(native["sha1"])
    HEADER.pack_into(
        control,
        0,
        MAGIC,
    ...
    header_crc = zlib.crc32(control[:HEADER_SIZE]) & 0xFFFFFFFF
    struct.pack_into("<I", control, HEADER.size - 4, header_crc)
...
def verify_expanded(data: bytes) -> dict:
    ...
    prefix_sha1 = sha1(data[:input_size])
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Build and verify the SAPPHIRE 32 MiB expanded ROM container"
    )
