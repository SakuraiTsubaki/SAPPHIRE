...
def audit_save(path: Path) -> dict:
    ...
    by_counter: dict[int, list[int]] = {}
    for sector in valid:
        by_counter.setdefault(sector["counter"], []).append(sector["section_id"])

    counter_groups = [
        {
            "counter": counter,
            "logical_ids": sorted(ids),
            "complete_0_13": sorted(ids) == list(range(14)),
        }
        for counter, ids in sorted(by_counter.items())
    ]
    ...
    return {
        ...
        "reserved_sectors": {
            "hall_of_fame": [
                {"sector": 28, "all_zero": sectors[28]["all_zero"]},
                {"sector": 29, "all_zero": sectors[29]["all_zero"]},
            ],
            "extension_candidates": [
                {"sector": 30, "all_zero": sectors[30]["all_zero"]},
                {"sector": 31, "all_zero": sectors[31]["all_zero"]},
            ],
        },
    }
...
def audit_directory(root: Path) -> dict:
    ...
    return {
        ...
        "all_sector_30_31_zero": all(
            all(x["all_zero"] for x in p["save"]["reserved_sectors"]["extension_candidates"])
            for p in pairs
        ),
        "pairs": pairs,
        "conclusions": {
            ...
            "save_hall_of_fame_sectors": "28-29; do not use",
            "save_extension_candidate_sectors": "30-31; A/B mirrored extension candidate",
            "save_extension_runtime_status": "static candidate only; expanded ROM read/write hooks still required",
        },
    }
